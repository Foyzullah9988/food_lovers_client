...
const Navbar = ({ fixed = true }) => {
    ...
    return (
        <nav className={`${fixed ? 'fixed top-0 left-0 w-full shadow-sm z-50' : 'relative'} bg-linear-to-r from-slate-900 via-slate-800 to-slate-900`}>
                        ...
                        <Link to={'/'} className="flex items-center space-x-3 group">
                            <div className="relative">
                                <div className="w-12 h-12 rounded-xl flex items-center justify-center  backdrop-blur-sm">
                                    <img
                                        src="/nav.png"
                                        className="md:w-10 md:h-10 w-7 h-7 object-cover rounded-lg"
                            ...
                            <div className="text-left">
                                <h2 className="md:text-2xl text-xl  font-bold tracking-tight">
                                    ...
                                    <span className="text-slate-100">Zone</span>
                                </h2>
                                <p className="text-xs hidden lg:block text-slate-400 tracking-wider">DISCOVER • TASTE • SHARE</p>
                            </div>
                        </Link>
                    </div>
                        ...
                        <div className="flex items-center justify-center space-x-1">
                            {allRoutes.map((route) => (
                                <NavLink
                                    ...
                                    className={({ isActive }) =>
                                        `flex items-center gap-2 px-4 py-2.5 rounded-lg transition-all duration-300 ${isActive
                                            ? 'bg-linear-to-r from-amber-600/20 to-amber-500/10 text-amber-300 border border-amber-500/30'
                        ...
                        {user && (
                            <>
                                ...
                                <div className="hidden xl:flex items-center gap-4">
                                    <div className="flex items-center gap-3 md:px-4 md:py-2 rounded-lg bg-slate-800/50 border border-slate-700">
                                        ...
                                        <div className="text-sm">
                                            ...
                                            <p className="text-xs text-slate-400">Food Critic</p>
                                        </div>
                                    </div>

                                    <button
                                        onClick={handleLogout}
                                        className="group flex items-center gap-2 px-4 py-2.5 rounded-lg bg-linear-to-r from-slate-800 to-slate-700 hover:from-slate-700 hover:to-slate-600 border border-slate-700 hover:border-amber-500/30 transition-all duration-300"
                                    >
                                        <MdOutlineLogout className="text-lg text-slate-300 group-hover:text-amber-300" />
                                        <span className="font-medium text-slate-300 group-hover:text-amber-200">Logout</span>
                                    </button>
                                </div>
                                    ...
                                    <div tabIndex={0} role="button" className="">
                                        <div className="w-10 h-10 rounded-full  border-2 border-amber-500/50">
                                            {user.photoURL ? (
                                                ...
                                                <div className="w-full h-full bg-linear-to-br from-amber-500 to-amber-600 flex items-center justify-center">
                                                    <MdPersonOutline className="text-xl text-white" />
                                                </div>
                                            )}
                                        </div>
                                    </div>
                                    <ul tabIndex={0} className="dropdown-content menu menu-compact bg-slate-900 rounded-box mt-3 w-56 p-2 shadow-xl border border-slate-700">
                                        <li className="menu-title p-3 border-b border-slate-800">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p className="font-semibold text-slate-100">{user.displayName || 'User'}</p>
                                                    <p className="text-xs text-slate-400">Food Critic</p>
                                                </div>
                                            </div>
                                        </li>
                                        ...
                                        {allRoutes.map((route) => (
                                            <li key={route.path}>
                                                <NavLink
                                                    ...
                                                    className={({ isActive }) =>
                                                        `text-slate-300 hover:bg-slate-800 hover:text-amber-200 rounded-lg flex items-center gap-2 ${isActive ? 'bg-slate-800 text-amber-300' : ''}`
                                                    }
                                                >
                                                    {route.icon}
                                                    {route.label}
                                        ...
                                        <div className="divider my-1 h-px bg-slate-800"></div>
                                        <li>
                                            <button
                                                onClick={handleLogout}
                                                className="text-slate-300 hover:bg-linear-to-r hover:from-amber-600/20 hover:to-amber-500/10 hover:text-amber-300 rounded-lg flex items-center gap-2"
                                            >
                        ...
                        {!user && (
                            <div className="xl:hidden">
                                <div className="dropdown dropdown-end">
                                    <div tabIndex={0} role="button" className="btn btn-ghost btn-circle text-slate-300 hover:text-amber-300">
